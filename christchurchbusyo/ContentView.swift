...
private extension URLRequest {
    mutating func addMetlinkHeaders() {
        self.setValue("application/json", forHTTPHeaderField: "Accept")
        self.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key")
    }
}
...
//        (Bundle.main.object(forInfoDictionaryKey: "WLApiKey") as? String) ?? ""
//    }
//}
    ...
    init(from decoder: Swift.Decoder) throws {
        ...
        if let boxed = try? c.decode([String:AnyDecodable].self),
           let d = boxed["data"], case let .array(arr) = d {
            // data: [...] 형태
            let dataJSON = try JSONSerialization.data(withJSONObject: arr, options: [])
            self.data = try? JSONDecoder().decode([T].self, from: dataJSON)
            return
        }
        // 루트 배열도 허용
        self.data = try? c.decode([T].self)
    }
}
...
private enum AnyDecodable: Decodable {
    ...
    init(from decoder: Swift.Decoder) throws {
        ...
        if let arr = try? c.decode([AnyDecodable].self) { self = .array(arr); return }
        if let dict = try? c.decode([String:AnyDecodable].self) { self = .dict(dict); return }
        self = .null
    }
}

// ---------- Stop Predictions ----------
 struct WLStopPredRaw: Decodable {
    // Metlink는 케이스가 제각각이라 후보 키 다 받음
    let stop_id: String?
    let minutes: Int?
...
// BusAPI.swift 상단 어딘가(ATAuth 옆)
enum WLAuth {
    /// Metlink Open Data 포털에서 받은 x-api-key
    static var apiKey: String = "2ErAk4g7359cn8AD8nUop8Wv9xE1ykK88JfBbNcr"
}

//private enum WLAuth {
//    static let base = "https://api.opendata.metlink.org.nz"
//    static let apiKey: String = {
//        // Info.plist > MetlinkAPIKey
//        if let k = Bundle.main.object(forInfoDictionaryKey: "MetlinkAPIKey") as? String, !k.isEmpty {
//            return k
//        }
//        assertionFailure("❌ Metlink API key missing. Add 'MetlinkAPIKey' to Info.plist.")
//        return ""
//    }()
//}
// MARK: - App
...
final class BusAPI: NSObject, URLSessionDelegate {
    ...
    private var _chcStopsLoaded = false

    
    // 캐시 (앱 생애 동안 1회만 decode)
       private static var _chcStopsCache: [BusStop]?

       /// 번들에 복사한 chc_stops.json을 읽어서, 중심 반경 내 정류장만 반환
       /// - Parameters:
       ///   - center: 지도 중심
       ///   - radiusMeters: 표시 반경(기본 1200m)
       /// - Returns: BusStop 배열
       func chc_fetchStopsFromBundle(center: CLLocationCoordinate2D,
                                     radiusMeters: Double = 1200) async -> [BusStop] {
           // 1) 캐시 없으면 로드
           if Self._chcStopsCache == nil {
               guard let url = Bundle.main.url(forResource: "chc_stops", withExtension: "json") else {
                   print("❌ [CHC Stops] bundle file not found: chc_stops.json (Target Membership 확인)")
                   return []
               }
               do {
                   let data = try Data(contentsOf: url)
                   // JSON 스키마: [{ "id": "...", "name": "...", "lat": ..., "lon": ... }, ...]
                   struct CHCStop: Decodable { let id: String; let name: String; let lat: Double; let lon: Double }
                   let raw = try JSONDecoder().decode([CHCStop].self, from: data)

                   let mapped: [BusStop] = raw.map {
                       BusStop(id: $0.id, name: $0.name, lat: $0.lat, lon: $0.lon, cityCode: 0)
                   }
                   Self._chcStopsCache = mapped
                   print("📦 [CHC Stops] loaded from bundle: total=\(mapped.count)")
               } catch {
                   let head = (try? String(contentsOf: url)).map { String($0.prefix(240)) } ?? ""
                   print("❌ [CHC Stops] decode error: \(error)\n🧾 head='\(head)'")
                   return []
               }
           }

           // 2) 거리 필터링
           guard let all = Self._chcStopsCache, !all.isEmpty else { return [] }
           let here = CLLocation(latitude: center.latitude, longitude: center.longitude)

           let filtered = all
               .map { s -> (BusStop, CLLocationDistance) in
           ...
           print("📍 [CHC Nearby] center=(\(center.latitude),\(center.longitude)) radius=\(Int(radiusMeters))m → \(filtered.count)")
           return filtered
       }
    /// 번들 파일(chc_stops.json)에서 정류장 목록을 한 번만 읽어 캐시합니다.
    /// 파일 포맷(JSON 배열): [{"id":"53340","name":"Bus Interchange","lat":-43.5352,"lon":172.6390}, ...]
    func chc_loadStopsFromBundle() -> [BusStop] {
        if _chcStopsLoaded { return _chcStopsCache }
        _chcStopsLoaded = true
        defer { print("📦 [CHC Stops] loaded=\(_chcStopsCache.count)") }

        guard let url = Bundle.main.url(forResource: "chc_stops", withExtension: "json") else {
            // 🔁 번들 파일이 없으면 최소 시드라도 넣어 지도에 보이게
            print("⚠️ [CHC Stops] bundle file not found (chc_stops.json). Using tiny seed.")
            _chcStopsCache = [
                BusStop(id: "53340", name: "Bus Interchange", lat: -43.5349, lon: 172.6390, cityCode: 0),
                BusStop(id: "53153", name: "Manchester St",  lat: -43.5339, lon: 172.6399, cityCode: 0),
                BusStop(id: "53349", name: "Lichfield St",   lat: -43.5357, lon: 172.6398, cityCode: 0),
            ]
            return _chcStopsCache
        }

        do {
            let data = try Data(contentsOf: url)
            let arr = try JSONDecoder().decode([BusStop].self, from: data)
            _chcStopsCache = arr
            return arr
        } catch {
            print("❌ [CHC Stops] bundle decode fail: \(error)")
            _chcStopsCache = []
            return []
        }
    }

    /// 중심/반경으로 번들 캐시에서 근처 정류장을 추립니다.
    func chc_nearbyStopsFromBundle(center: CLLocationCoordinate2D,
                                   radiusMeters: CLLocationDistance = 800,
                                   limit: Int = 400) -> [BusStop] {
        let all = chc_loadStopsFromBundle()
    ...
    // === 추가: Christchurch 전용 우회 ===
    func cc_fetchStopsDisabled(lat: Double, lon: Double) async -> [BusStop] {
        print("🚫 [CHC Stops] nearby-by-geo not supported on CHC. Returning []. lat=\(lat) lon=\(lon)")
        return []
    }
    // Christchurch: Stop Monitoring (SIRI)
    // http://apis.metroinfo.co.nz/rti/siri/v1/sm?stopcode=53340
    ...
    /// 기존 호출이 남아있을 수 있어 '안전 스텁'으로 교체합니다.
    func chc_fetchStops(lat: Double, lon: Double, radiusMeters: Int = 700) async throws -> [BusStop] {
        // CHC는 위경도 근처 정류장 검색 엔드포인트가 없음.
           print("🚫 [CHC Stops] geo search NOT supported. lat=\(lat) lon=\(lon) radius=\(radiusMeters)")
           return []
    }

    // 필요 시 인증 키
        enum CHCAuth {
            // Info.plist → "CHCSubscriptionKey" 권장
            static var subscriptionKey: String = {
                if let k = Bundle.main.object(forInfoDictionaryKey: "CHCSubscriptionKey") as? String, !k.isEmpty { return k }
                return "ffbd7cc07cae4e88aeba3fdaf9b2a279" // ← 개발 중 임시 (사진 스펙의 키를 넣으세요)
            }()
        }

        // 공통 GET
        private func chcGET(_ path: String, query: [URLQueryItem] = []) async throws -> Data {
            // 사진 스펙 기준 base 채우기
            let base = "https://apis.metroinfo.co.nz" // ex) api.metroinfo.co.nz / realtimetransit...
            var comp = URLComponents(string: base + path)!
            if !query.isEmpty { comp.queryItems = query }
            var req = URLRequest(url: comp.url!)
            req.setValue("application/json", forHTTPHeaderField: "Accept")
            if !CHCAuth.subscriptionKey.isEmpty {
                req.setValue(CHCAuth.subscriptionKey, forHTTPHeaderField: "Ocp-Apim-Subscription-Key")
            }
            let (data, resp) = try await URLSession.shared.data(for: req)
            let http = resp as! HTTPURLResponse
            print("📡 [CHC HTTP \(http.statusCode)] \(path) q=\(query.map{$0.name}.joined(separator: ",")) bytes=\(data.count)")
            return data
        }

        // 4-1) 정류장 조회 (반경 검색)
        func cc_fetchStops(lat: Double, lon: Double) async throws -> [BusStop] {
            // 스펙에 맞는 path/파라미터로 교체
            let data = try await chcGET("/stops/nearby", query: [
                .init(name: "lat", value: String(lat)),
                .init(name: "lon", value: String(lon)),
                .init(name: "radius", value: "800")
            ])

            struct CCStop: Decodable { let id: String; let name: String; let lat: Double; let lon: Double }
            // 사진 스펙 구조에 맞춰 아래 매핑 수정
            struct Root<T: Decodable>: Decodable { let data: [T]? }

            let decoded = (try? JSONDecoder().decode(Root<CCStop>.self, from: data))?.data ?? []
            let out = decoded.map { BusStop(id: $0.id, name: $0.name, lat: $0.lat, lon: $0.lon,cityCode: 0) }
            print("✅ [CHC Stops] in=\(decoded.count) out=\(out.count) sample=\(out.prefix(3).map{$0.name})")
            return out
        }

        // 4-2) 차량 위치 (전 노선 일괄)
        func cc_fetchVehiclePositions() async throws -> [BusLive] {
            let data = try await chcGET("/realtime/vehiclepositions") // ← 스펙 path로 교체
            // 사진 스펙 구조에 맞춰 타입 작성
            struct CCVeh: Decodable {
                let id: String
                let route: String
                let latitude: Double
            ...
            let arr = (try? JSONDecoder().decode(Root<CCVeh>.self, from: data))?.data ?? []
            let out: [BusLive] = arr.map {
                BusLive(id: $0.id, routeNo: $0.route.uppercased(),
                        lat: $0.latitude, lon: $0.longitude,
                        etaMinutes: nil, nextStopName: nil)
            }
            print("✅ [CHC Vehicles] in=\(arr.count) out=\(out.count) routes=\(Set(out.map{$0.routeNo}).count)")
            return out
        }

        // 4-3) 정류장 도착예정(ETA)
        func cc_fetchStopPredictions(stopIds: [String]) async throws -> [ArrivalInfo] {
            guard !stopIds.isEmpty else { return [] }
            // 스펙상 배치 가능하면 ids[]=.. 식으로, 아니면 first만 루프
            var out: [ArrivalInfo] = []
            for sid in stopIds {
                let data = try await chcGET("/realtime/stop/\(sid)/predictions") // ← 스펙 path로 교체
                struct CCPred: Decodable {
                    let route: String
                    let etaMinutes: Int
                    // 필요 시 destination 등 추가
                }
                struct Root<T: Decodable>: Decodable { let data: [T]? }

                let arr = (try? JSONDecoder().decode(Root<CCPred>.self, from: data))?.data ?? []
                let mapped = arr.map {
                    ArrivalInfo(routeId: $0.route.uppercased(),
                                routeNo: $0.route.uppercased(),
                                etaMinutes: max(0, $0.etaMinutes))
                }
                print("ℹ️ [CHC ETA] stop=\(sid) items=\(mapped.count)")
                out.append(contentsOf: mapped)
            }
            print("✅ [CHC ETA] stops=\(stopIds.count) total=\(out.count)")
            return out
        }
    ...
    func wl_requestLogged(_ path: String, note: String) async throws -> (Data, HTTPURLResponse) {
            let url = URL(string: "https://api.opendata.metlink.org.nz\(path)")!
            var req = URLRequest(url: url)
            req.setValue("application/json", forHTTPHeaderField: "Accept")
            req.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key")

            let t0 = Date()
            let (d, r) = try await URLSession.shared.data(for: req)
            let ms = Int(Date().timeIntervalSince(t0) * 1000)

            let code = (r as! HTTPURLResponse).statusCode
            let bodyPreview = String(data: d, encoding: .utf8)?.prefix(280) ?? "∅"
            print("🔌 [WL \(note)] \(code) \(ms)ms \(url.absoluteString)\n    body[0..280]=\(bodyPreview)")

            return (d, r as! HTTPURLResponse)
        }

        /// JSON 루트의 data/배열 길이와 stop_id/route 샘플만 뽑아보기(파서 전)
        func wl_quickPeek(_ data: Data, label: String) {
            guard let obj = try? JSONSerialization.jsonObject(with: data) else {
                print("👀 [WL \(label)] JSON parse fail")
                return
            }
            if let dict = obj as? [String: Any], let arr = dict["data"] as? [[String: Any]] {
                let n = arr.count
                let sample = arr.prefix(3).map { it -> String in
                    let sid = it["stop_id"] ?? it["stopId"] ?? "∅"
                    let rn  = it["service_name"] ?? it["route_short_name"] ?? it["route_name"] ?? it["route_id"] ?? "∅"
                    return "{stop_id=\(sid), route=\(rn)}"
                }
                print("👀 [WL \(label)] data.count=\(n) sample=\(sample)")
                ...
                print("👀 [WL \(label)] array.count=\(n) sample=\(sample)")
            } else {
                print("👀 [WL \(label)] unknown root shape: \(type(of: obj))")
            }
        }
    // BusAPI.swift 내부(private helpers 섹션)
    private func wlGET(_ url: URL) async throws -> (Data, HTTPURLResponse) {
        var req = URLRequest(url: url)
        req.httpMethod = "GET"
        req.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key")     // 🔑 필수
        req.setValue("application/json", forHTTPHeaderField: "Accept")
        ...
        if http.statusCode != 200 {
            // 디버그에 보이는 403 래핑
            struct WLHTTPError: Error { let status: Int; let url: URL }
            print("❌ [Metlink] \(http.statusCode) \(url.absoluteString)")
            throw WLHTTPError(status: http.statusCode, url: url)
        }
        return (data, http)
    }
    ...
    struct WLHTTPError: Error { let status: Int; let url: URL }
       ...
       @discardableResult
       fileprivate func wl_sendJSON(_ tag: String, url: URL) async throws -> (Data, HTTPURLResponse) {
           var req = URLRequest(url: url)
           req.httpMethod = "GET"
           // 🔑 반드시 필요: Metlink 인증
           req.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key")
           ...
           if !(200...299).contains(http.statusCode) {
               print("❌ [Metlink:\(tag)] \(http.statusCode) \(url.absoluteString)")
               throw WLHTTPError(status: http.statusCode, url: url)
           }
           return (data, http)
       }
    
    enum AuthKind { case none, xApiKey(String) }

       fileprivate func applyAuth(_ kind: AuthKind, to req: inout URLRequest) {
           switch kind {
           case .none: break
           case .xApiKey(let k): req.setValue(k, forHTTPHeaderField: "x-api-key")
           }
           req.setValue("BusApp/1.0 iOS", forHTTPHeaderField: "User-Agent")
           req.setValue("gzip", forHTTPHeaderField: "Accept-Encoding")
       }

       fileprivate func assertOK(_ resp: URLResponse) throws {
    ...
    // MARK: - Wellington: 근처 정류장
    // BusAPI.swift
    func wl_fetchStops(lat: Double, lon: Double, radiusMeters: Double = 600) async throws -> [BusStop] {
            let url = URL(string: "https://api.opendata.metlink.org.nz/v1/gtfs/stops")!
            var req = URLRequest(url: url)
            req.addMetlinkHeaders()

            let (data, resp) = try await URLSession.shared.data(for: req)
            guard let http = resp as? HTTPURLResponse, http.statusCode == 200 else {
                throw WLHTTPError(status: (resp as? HTTPURLResponse)?.statusCode ?? -1, url: url)
            }

            

            // 응답 형식: [WLStop] 또는 { "data": [WLStop] } 둘 다 수용
            if let wrapped = try? JSONDecoder().decode([String:[WLStop]].self, from: data),
               let arr = wrapped["data"] {
                return Self._filterStops(arr, lat: lat, lon: lon, radiusMeters: radiusMeters)
            } else {
                let arr = try JSONDecoder().decode([WLStop].self, from: data)
                return Self._filterStops(arr, lat: lat, lon: lon, radiusMeters: radiusMeters)
            }
        }

    private static func _filterStops(
        _ src: [WLStop],
        lat: Double,
        lon: Double,
        radiusMeters: Double
        ...
        return src.compactMap { (s: WLStop) -> BusStop? in
            guard
                let id   = s.stop_id,
                let name = s.stop_name,
                let la   = s.stop_lat,
                let lo   = s.stop_lon
            else { return nil }
    ...
    // MARK: - Wellington: 정류장 ETA
  
        /// 웰링턴 정류장 도착예측
    // MARK: - Wellington: Stop predictions (batch by stop_id)
    struct WLStopPrediction {
           let routeNo: String
           let etaMinutes: Int
           let destination: String?
           let stopId: String
    ...
    // BusAPI.swift
        ...
        /// 여러 stop_id를 한번에 조회 (stop_id=... 반복)
    // BusAPI.swift 안에 이 메서드를 "통째로" 교체
    // BusAPI.swift

    // ISO8601 duration("PT7M19S" 등) → 분(Int) 변환
//    private func minutesFromISODuration(_ s: String?) -> Int? {
//        guard let s else { return nil }
//        // 아주 관대한 파서 (P[n]DT[n]H[n]M[n]S 중 분/초만 사용)
//        var m = 0, sec = 0
//        // M, S 추출
//        if let r = try? NSRegularExpression(pattern: "PT(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?") {
//            let str = s as NSString
//            if let m0 = r.firstMatch(in: s, range: NSRange(location: 0, length: str.length)) {
//                let hStr = m0.range(at: 1).location != NSNotFound ? str.substring(with: m0.range(at: 1)) : nil
//                let mStr = m0.range(at: 2).location != NSNotFound ? str.substring(with: m0.range(at: 2)) : nil
//                let sStr = m0.range(at: 3).location != NSNotFound ? str.substring(with: m0.range(at: 3)) : nil
//                let h  = Int(hStr ?? "0") ?? 0
//                let mm = Int(mStr ?? "0") ?? 0
//                sec    = Int(sStr ?? "0") ?? 0
//                m = h * 60 + mm
...
//    }
    ...
    /// Wellington: 여러 stop_id에 대한 예측을 한 번에 받아서 ArrivalInfo로 리턴
    // BusAPI.swift

        /// Wellington: 정류장별 출발 예측을 ArrivalInfo로 변환
        /// - Parameter stopIds: Metlink 정류장 id 문자열 배열 (예: ["5513","6708"])
        /// - Returns: ETA(분) 기준 오름차순의 ArrivalInfo 목록 (각 항목은 service_id를 routeNo/routeId로 사용)
        func wl_fetchStopPredictions(stopIds: [String]) async throws -> [ArrivalInfo] {

            struct Root: Decodable {
                let departures: [Departure]?
            }
            struct NameObj: Decodable {
                let stop_id: String?
                let name: String?
            }
            struct Departure: Decodable {
                // 핵심
                let stop_id: String?
                let service_id: String?        // ← routeNo/routeId로 사용
                let destination: NameObj?
                // 시간 관련(버전에 따라 필드명이 다를 수 있어 방어적으로 모두 시도)
                let aimed_departure: String?   // ISO8601 or "HH:MM:SS"
                let expected_departure: String?
                let display_departure: String? // "5 min", "DUE", etc
                let departure: String?         // 일부 변종
                let departure_time: String?    // 일부 변종
                let delay: String?             // ISO8601 duration "PT23M41S"
            }

            func req(for stopId: String) -> URLRequest {
                var r = URLRequest(url: URL(string: "https://api.opendata.metlink.org.nz/v1/StopDepartures/\(stopId)")!)
                r.setValue("application/json", forHTTPHeaderField: "Accept")
                r.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key") // 🔑 프로젝트에 정의해두세요
                return r
            }

            // ETA 파서(여러 포맷 방어)
            func parseETAMinutes(from d: Departure, now: Date) -> Int? {
                // 1) expected_departure/aimed_departure/… 절대시각
                let timeStrings: [String?] = [
                    d.expected_departure, d.aimed_departure, d.departure, d.departure_time
                ]
                for ts in timeStrings {
                    if let s = ts, let t = parseDateFlexible(s) {
                        let sec = Int(t.timeIntervalSince(now))
                        if sec >= 0 { return sec / 60 }
                    }
                }
                // 2) display_departure "5 min" / "DUE" / "Delayed"
                if let s = d.display_departure?.lowercased() {
                    if let m = firstInt(in: s) { return max(0, m) }
                    if s == "due" { return 0 }
                }
                // 3) delay ISO8601 duration "PT23M41S" → 분으로 환산(보수적)
                if let z = d.delay, let dur = parseISODurationToSeconds(z) {
                    return max(0, dur / 60)
                }
                return nil
            }

            func parseDateFlexible(_ s: String) -> Date? {
                // ISO8601 (예: 2025-02-20T18:43:00Z)
                let iso = ISO8601DateFormatter()
                iso.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
                if let d = iso.date(from: s) ?? ISO8601DateFormatter().date(from: s) { return d }
                // HH:mm[:ss] (오늘 기준, 뉴질랜드 시간대)
                let comps = s.split(separator: ":").map(String.init)
                if comps.count >= 2, let hh = Int(comps[0]), let mm = Int(comps[1]) {
                    var dc = Calendar(identifier: .gregorian).dateComponents(in: TimeZone(identifier: "Pacific/Auckland")!, from: Date())
                    dc.hour = hh; dc.minute = mm; dc.second = (comps.count >= 3 ? Int(comps[2]) ?? 0 : 0)
            ...
            func firstInt(in s: String) -> Int? {
                ...
                guard !digits.isEmpty, let m = Int(String(digits)) else { return nil }
                return m
            }

            func parseISODurationToSeconds(_ s: String) -> Int? {
                // 아주 단순 파서: PT#H#M#S 일부만 지원
                guard s.hasPrefix("P") else { return nil }
                let t = s.replacingOccurrences(of: "PT", with: "")
                var sec = 0
                var buf = ""
                for ch in t {
            ...
            try await withThrowingTaskGroup(of: [ArrivalInfo].self) { group in
                for sid in stopIds {
                    group.addTask {
                        var out: [ArrivalInfo] = []
                        do {
                            let (data, resp) = try await URLSession.shared.data(for: req(for: sid))
                            guard let http = resp as? HTTPURLResponse, (200...299).contains(http.statusCode) else { return out }
                            let root = try JSONDecoder().decode(Root.self, from: data)
                            let deps = root.departures ?? []
                            for dep in deps {
                                guard let svc = dep.service_id?.trimmingCharacters(in: .whitespacesAndNewlines),
                                      !svc.isEmpty
                                else { continue }

                                // ETA 계산(분)
                                guard let etaMin = parseETAMinutes(from: dep, now: now) else { continue }
                                guard etaMin <= 40 else { continue }             // ✅ 60분 이내만

                                let routeNo = svc.uppercased()
                                let routeId = routeNo                              // ✅ veh.routeNo와 동일 키
                                let dest = dep.destination?.name

                                out.append(ArrivalInfo(
                                    routeId: routeId,
                                    routeNo: routeNo,
                                    etaMinutes: max(0, etaMin)
                                ))
                            }
                        } catch {
                            // 조용히 실패 → 빈 배열 반환
            ...
            // ETA 오름차순
            results.sort { $0.etaMinutes < $1.etaMinutes }
            return results
        }
    ...
    // Wellington stop-predictions attributes (FlexString 사용)
    struct WLStopPredictionAttrs: Decodable {
        let stop_id: FlexString?
        let route_id: FlexString?
        let service_name: String?
        let route_short_name: String?
        let route_name: String?
        let minutes: Int?
    ...
    private func wl_request(_ path: String) async throws -> (Data, HTTPURLResponse) {
           let url = URL(string: "https://api.opendata.metlink.org.nz\(path)")!
           var req = URLRequest(url: url)
           req.setValue("application/json", forHTTPHeaderField: "Accept")
        req.setValue(WLAuth.apiKey, forHTTPHeaderField: "x-api-key")
       ...
       /// 전체 정류장 예측(필터는 VM에서)
    func wl_fetchAllStopPredictions() async throws -> [WLStopPredRaw] {
        let (data, resp) = try await wl_requestLogged("/v1/stop-predictions", note: "predictions")
        guard (200...299).contains(resp.statusCode) else {
            return []
//            throw WLHTTPError(status: resp.statusCode, url: resp.url?.absoluteString ?? "")
        }
        wl_quickPeek(data, label: "predictions")

        if let env = try? JSONDecoder().decode(WLEnvelope<WLStopPredRaw>.self, from: data),
           let arr = env.data { return arr }
        if let arr = try? JSONDecoder().decode([WLStopPredRaw].self, from: data) { return arr }
        return []
    }


    // MARK: - Wellington: 차량 위치 (간단 JSON 버전 가정)
    // GTFS-RT(프로토버퍼) 쓰는 경우는 별도 파서 필요. JSON 제공 엔드포인트가 있으면 그걸 사용.
    // BusAPI.swift
    private struct WLVPEnvelope: Decodable { let entity: [WLEntity] }
       private struct WLEntity: Decodable {
           let id: String?
           let vehicle: WLVehicle?
       }
       private struct WLVehicle: Decodable {
           let position: WLPos?
           let trip: WLTrip?
           let vehicle: WLVehId?
       }
       private struct WLPos: Decodable { let latitude: Double?; let longitude: Double? }
       private struct WLTrip: Decodable { let route_id: StringOrInt? }
       private struct WLVehId: Decodable { let id: String? }

       /// 웰링턴 차량 위치
    private struct WLVehRaw: Decodable {
            // id
            ...
            // 중첩 케이스 수용
            let vehicle: Vehicle?
            struct Vehicle: Decodable { let position: Position? }
        ...
        /// 모든 차량 위치(한 번에)
    // MARK: - Wellington: Vehicle positions (GTFS-RT JSON proxy)
    // MARK: - Metlink: vehicle positions
    func wl_fetchVehiclePositions() async throws -> [BusLive] {
        ...
        struct Feed: Decodable {
            struct Entity: Decodable {
                ...
                struct VehicleWrapper: Decodable {
                    struct Trip: Decodable { let route_id: FlexString? } // ← 숫자/문자 혼용 대응
                    ...
                    struct VehicleId: Decodable { let id: String?; let label: String? }
                    let trip: Trip?
                    let position: Position?
                    let vehicle: VehicleId?
                    let timestamp: Int?
                }
                let vehicle: VehicleWrapper?
            }
            let entity: [Entity]?
        }

        let feed = try JSONDecoder().decode(Feed.self, from: data)
        let ents = feed.entity ?? []

        var out: [BusLive] = []
        out.reserveCapacity(ents.count)
        ...
        for e in ents {
            ...
            // 노선 라벨(아이콘 표기와 동일 규칙)
            let ridRaw  = v.trip?.route_id?.value ?? "?"
            let routeNo = canonicalRouteNo(ridRaw)

            // 차량 id (중복 방지용 원시 id)
            let vehRaw = v.vehicle?.id ?? e.id ?? UUID().uuidString

            out.append(BusLive(
                id: vehRaw,           // NOTE: mergeAndFilter에서 routeId와 합성ID로 다시 씀
                routeNo: routeNo,     // ← 아이콘/ETA 키 모두 이 값으로
                lat: lat,
                lon: lon,
                etaMinutes: nil,
                nextStopName: nil
        ...
        print("🚌 [WL vehicles] mapped=\(out.count)")
        return out
    }
...
final class BusAnnotation: NSObject, MKAnnotation {
    ...
    // 맵에 실제로 올라가 있는지(추가 직후 true, 제거 직전 false)
        @objc dynamic private(set) var isOnMap: Bool = false
...
//       }
    ...
    func refreshFocusStopETA() async {
        ...
        case .wellington:
            do {
                print("🔎 [WL Focus] fetch ETA for stop id=\(s.id) name=\(s.name)")

                // ---- pass #1: 해당 정류소 단독
                var arrivals: [ArrivalInfo] = try await api.wl_fetchStopPredictions(stopIds: [s.id])
                print("ℹ️ [WL Focus] pass#1 arrivals=\(arrivals.count)  routes=\(Set(arrivals.map{$0.routeNo}))")

                // ---- pass #2: 인접 정류소까지 합산 (반경 80m)
                if arrivals.isEmpty {
                    let neighborIds = nearestNeighborStopIds(of: s, withinMeters: 80, limit: 3, includeSelf: true)
                    if neighborIds.count > 1 {
                        print("🔁 [WL Focus] retry with neighbors ids=\(neighborIds)")
                        let a2 = try await api.wl_fetchStopPredictions(stopIds: neighborIds)
                        print("ℹ️ [WL Focus] pass#2 arrivals=\(a2.count)")
                        arrivals.append(contentsOf: a2)
                    }
                }

                // ---- pass #3: 차량기반 추정 (최후 폴백)
                if arrivals.isEmpty {
                    print("⚠️ [WL Focus] API empty → fallback to vehicle estimation")
                    let est = await estimateArrivalsFromVehiclesWL(stop: s, maxPerRoute: 2)
                    arrivals = est
                }

                // ---- 정규화 (노선번호, ETA 음수 방지)
                let normalized: [ArrivalInfo] = arrivals.map { a in
                    var m = a
                    m.routeNo = canonicalRouteNo(a.routeNo)
                    m.etaMinutes = max(0, a.etaMinutes)
                    return m
                }

                // ---- routeNo 단위 그룹핑
                let grouped = Dictionary(grouping: normalized, by: { $0.routeNo })

                var compact: [ArrivalInfo] = []
                for (rno, items) in grouped {
                    // 같은 ETA 분(minute) 값은 1개만 남김 → 중복 제거
                    var byMin: [Int: ArrivalInfo] = [:]
                    for it in items {
                        if byMin[it.etaMinutes] == nil { byMin[it.etaMinutes] = it }
                    }

                    // ETA 오름차순으로 정렬 후 상위 2개까지만
                    let top = byMin.values
                        .sorted { $0.etaMinutes < $1.etaMinutes }
                        .prefix(2)

                    compact.append(contentsOf: top)
                }

                // ---- 전체 필터/정렬/상한
                let maxEtaMin = 40      // ETA 40분 초과는 숨김
                let maxDisplay = 20     // 전체 최대 표시 개수
                let finalList = compact
                    .filter { $0.etaMinutes <= maxEtaMin }
                    .sorted { $0.etaMinutes < $1.etaMinutes }
                    .prefix(maxDisplay)

                self.focusStopETAs = Array(finalList)

                // ---- 로그 출력
                let sample = finalList.map { "\($0.routeNo)\($0.etaMinutes)m" }.joined(separator: ", ")
                print("✅ [WL Focus] groupedRoutes=\(grouped.keys.count) final=\(finalList.count) → \(sample)")

            } catch {
                print("❌ [WL Focus] error: \(error)")
                self.focusStopETAs = []
            }
            // MapVM.refreshFocusStopETA() 안의 switch provider { ... } 에 추가
            // MapVM.swift
            // refreshFocusStopETA() 안의 switch provider { case .christchurch: ... } 를 "통째로" 교체
            case .christchurch:
                do {
                    guard let s = self.focusStop else {
                        print("🚫 [CHC Focus] no focusStop")
                        self.focusStopETAs = []
                        return
                    }
                    let stopCode = s.id      // CHC에선 id를 stopcode로 사용
                    print("🔎 [CHC Focus] fetch ETA via SIRI sm for stopCode=\(stopCode) name=\(s.name)")

                    // 1) SIRI StopMonitoring 호출
                    let raw = try await api.chc_fetchArrivals(stopCode: stopCode)
                    print("ℹ️ [CHC Focus] siri items=\(raw.count) routes=\(Set(raw.map{$0.routeNo}))")

                    // 2) 정규화(노선 표기/ETA 보정)
                    let normalized = raw.map { a -> ArrivalInfo in
                        var m = a
                        m.routeNo = canonicalRouteNo(a.routeNo)
                        m.etaMinutes = max(0, a.etaMinutes)
                        return m
                    ...
                    // 3) 라우트별 상위 2개만(같은 분 중복 제거)
                    let grouped = Dictionary(grouping: normalized, by: { $0.routeNo })
                    var compact: [ArrivalInfo] = []
                    for (_, items) in grouped {
                        var seen = Set<Int>()
                        for it in items.sorted(by: { $0.etaMinutes < $1.etaMinutes }) {
                            if seen.insert(it.etaMinutes).inserted { compact.append(it) }
                    ...
                    // 4) 최종 컷(<=40분, 최대 20개)
                    let finalList = compact
                        .filter { $0.etaMinutes <= 40 }
                        .sorted { $0.etaMinutes < $1.etaMinutes }
                        .prefix(20)

                    self.focusStopETAs = Array(finalList)
                    print("✅ [CHC Focus] final=\(finalList.count) → \(finalList.map { "\($0.routeNo)\($0.etaMinutes)m" }.joined(separator: ", "))")

                } catch {
                    print("❌ [CHC Focus] error: \(error)")
                    self.focusStopETAs = []
                }
    ...
    private func canonicalizeWLBus(_ b: BusLive) -> BusLive {
        var m = b
        m.routeNo = canonicalRouteNo(b.routeNo)
        return m
    }

    /// 웰링턴: API가 빈 값을 줄 때, 현재 차량으로 ETA를 "추정"해서라도 채워준다.
    /// - 원리: 반경 1.2km 내, 정류소 쪽으로 향하는 차량을 고르고
    ///         (속도 관측치 없으면 7 m/s 가정), 거리/속도로 ETA 계산.
    /// - 결과: routeNo별 최대 2개씩 반환.
    private func estimateArrivalsFromVehiclesWL(stop: BusStop, maxPerRoute: Int) async -> [ArrivalInfo] {
        ...
        // ① 현재 보유 차량이 있으면 우선 사용, 없으면 1회 조회
        var busesNow = self.buses
        if busesNow.isEmpty {
            if let fetched = try? await api.wl_fetchVehiclePositions() {
                busesNow = fetched
            }
        }
        if busesNow.isEmpty { return [] }

        let candRadius: CLLocationDistance = 1200
        let minHeadingDot: Double = 0.0   // 정류장 방향으로 90도 이내(느슨)
        let vDefault: Double = 7.0        // m/s (약 25km/h)
        let etaCapMin = 90

        // dir 단위벡터(버스→정류장)
        func dirUnit(from a: CLLocationCoordinate2D, to b: CLLocationCoordinate2D) -> (x: Double, y: Double) {
            let v = GeoUtil.deltaMeters(from: a, to: b)
            let d = max(1e-6, v.dist)
            return (v.dx/d, v.dy/d)
        }

        var estimates: [ArrivalInfo] = []
        for raw in busesNow {
            let b = canonicalizeWLBus(raw)
            let here = CLLocationCoordinate2D(latitude: b.lat, longitude: b.lon)
            let dist = stopLoc.distance(from: CLLocation(latitude: here.latitude, longitude: here.longitude))
            if dist > candRadius { continue }

            // 진행방향 추정(최근 트랙이 있으면 사용)
            let vObs = tracks[b.id]?.speedMps ?? 0
            let vForETA = max(1.5, vObs > 0 ? vObs : vDefault)

            // 정류장 쪽으로 대략 향하는지(버스→정류장)·방향성 판단
            var headingOK = true
            if let dir = tracks[b.id]?.dirUnit {
                let targetDir = dirUnit(from: here, to: stopCoord)
                let dot = dir.x * targetDir.x + dir.y * targetDir.y
                headingOK = (dot >= minHeadingDot)
            }
            ...
            let etaMin = min(etaCapMin, max(0, Int((Double(etaSec)/60.0).rounded(.toNearestOrEven))))

            estimates.append(ArrivalInfo(routeId: b.routeNo,   // WL에선 routeId=routeNo로 취급
                                         routeNo: b.routeNo,
                                         etaMinutes: etaMin))
        }

        // routeNo별 상위 maxPerRoute개만
        let grouped: [String: [ArrivalInfo]] = Dictionary(grouping: estimates, by: { $0.routeNo })
        var compact: [ArrivalInfo] = []
        for (rno, items) in grouped {
            let sorted = items.sorted { $0.etaMinutes < $1.etaMinutes }
            compact.append(contentsOf: sorted.prefix(maxPerRoute))
        }
        compact.sort { $0.etaMinutes < $1.etaMinutes }
        return compact
    }

    private func nearestNeighborStopIds(of stop: BusStop,
                                        withinMeters r: CLLocationDistance,
                                        limit: Int,
                                        includeSelf: Bool) -> [String] {
        let here = CLLocation(latitude: stop.lat, longitude: stop.lon)
        // 지도에 보이는 것 + 전역 캐시 모두 사용
        var all: [BusStop] = stops
        for s in knownStopsIndex.values { if !all.contains(where: { $0.id == s.id }) { all.append(s) } }

        var cands: [(id: String, d: CLLocationDistance)] = []
        for s in all {
            let d = here.distance(from: CLLocation(latitude: s.lat, longitude: s.lon))
            if d <= r {
                if includeSelf || s.id != stop.id {
                    cands.append((s.id, d))
                }
            }
        }
        let sorted = cands.sorted { $0.d < $1.d }
        var ids = sorted.map { $0.id }
        if includeSelf && !ids.contains(stop.id) { ids.insert(stop.id, at: 0) }
        if !includeSelf { ids = ids.filter { $0 != stop.id } }
        // 중복 제거
    ...
    /// WLStopPrediction → ArrivalInfo 매핑 + 60분 필터 + routeNo 정규화
    /// WLStopPrediction → ArrivalInfo 매핑 + 60분 필터 + routeNo 정규화
    private func wl_mapPredsToArrivals(_ preds: [BusAPI.WLStopPrediction]) -> [ArrivalInfo] {
        // ⏱️ 60분 이내만 남기기
        let capped = preds.filter { max(0, $0.etaMinutes) <= 40 }

        // 매핑 (Wellington에서는 routeId = routeNo로 사용)
        let mapped: [ArrivalInfo] = capped.map { p in
            let rn = canonicalRouteNo(p.routeNo)
            let minutes = max(0, p.etaMinutes)
            return ArrivalInfo(
                routeId: rn,
                routeNo: rn,
                etaMinutes: minutes
        ...
        // ETA 오름차순 정렬
        return mapped.sorted { $0.etaMinutes < $1.etaMinutes }
    }




    // 노선번호 정규화: "83-0" -> "83", "210" -> "21", 대소문자 통일
    private func canonicalRouteNo(_ raw: String) -> String {
        let t = raw.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        if let i = t.firstIndex(of: "-") { return String(t[..<i]) }
        return t
    }
        ...
        func focusETACompactSummary() -> String {
            guard !focusStopETAs.isEmpty else { return "No arrival information" }
            let parts = focusStopETAs.prefix(5).map { "\($0.routeNo) \($0.etaMinutes)min" }
            return parts.joined(separator: " · ")
        }
    ...
    func reload(center: CLLocationCoordinate2D) async {
        ...
        do {
            ...
            switch provider {
            case .wellington:
                stops = try await api.wl_fetchStops(lat: center.latitude, lon: center.longitude)
            default:
                stops = try await api.chc_fetchStops(lat: center.latitude, lon: center.longitude)
            }
            ...
            switch provider {
                // MapVM.reload(center:) 안의 switch provider { ... } 에 아래 블록 추가
                ...
                case .christchurch:
                    do {
                        ...
                        // ② 차량 위치(Protobuf → BusLive)
                        let veh = try await api.chc_fetchVehiclePositions()
                        print("🛰️ [CHC Vehicles] raw=\(veh.count)")

                        // ③ 스냅샷/필터(경로 사영·점프 억제·ETA 추정 포함)
                        let snap = makeRouteSnapshot()
                        let filtered = self.mergeAndFilter(veh, snap: snap)
                        print("🧩 [CHC Vehicles] filtered=\(filtered.count)")

                        // (선택) routeNoById 갱신: 팔로우/라벨링에 필요하면 유지
                        for b in filtered { self.routeNoById[b.id] = b.routeNo }

                        // ④ 상태 반영 (초기부터 filtered 사용 → 트랙/ID 일관 유지)
                        applyIfCurrent(epoch: epoch) {
                            self.buses = filtered
                            self.latestTopArrivals = []   // ETA 패널은 정류장 탭 시 갱신
                        }
                        print("✅ [CHC] set buses=\(filtered.count)")
            ...
            case .wellington:
                // 1) 후보 정류장 선정
                let here = CLLocation(latitude: center.latitude, longitude: center.longitude)
                let candsWithDist: [(BusStop, CLLocationDistance)] = stops.map { s in
                    (s, here.distance(from: CLLocation(latitude: s.lat, longitude: s.lon)))
                }
                let candidates = candsWithDist
                    .filter { $0.1 <= 800 }
                    .sorted { $0.1 < $1.1 }
                    .prefix(6)
                    .map { $0.0 }

                let candIds = Array(Set(candidates.map { $0.id }))
                print("📍 [WL] candidates=\(candidates.count) ids=\(candIds) names=\(candidates.map{$0.name})")

                // 2) 후보 정류장 예측 → ArrivalInfo (60분 이내 필터 + canonical routeNo)
                let arrivals = try await api.wl_fetchStopPredictions(stopIds: candIds)
             // <- 매핑
                print("ℹ️ [WL] arrivals total=\(arrivals.count)  uniques(routeNo)=\(Set(arrivals.map{$0.routeNo}).count)")
                ...
                // 4) 차량 위치 (전 차량 일괄) + ETA 얹기
                let snap = makeRouteSnapshot()

                // routeNo(정규화) → 최소 ETA
                var etaByRoute: [String:Int] = [:]
                for a in top {
                    let key = canonicalRouteNo(a.routeNo)
                    etaByRoute[key] = min(etaByRoute[key] ?? Int.max, a.etaMinutes)
                }

                var mergedById: [String: BusLive] = [:]
                do {
                    let veh = try await api.wl_fetchVehiclePositions()
                    print("🛰️ [WL] vehicles raw=\(veh.count) sampleRoutes=\(Set(veh.prefix(10).map{$0.routeNo}))")

                    // ▶ 차량 노선번호도 canonical로 통일 + ETA 부여
                    let enriched = veh.map { b -> BusLive in
                        var m = b
                        m.routeNo = canonicalRouteNo(b.routeNo)     // 아이콘/ETA 키와 일치!
                        m.etaMinutes = etaByRoute[m.routeNo]
                        return m
                    }

                    let filtered = self.mergeAndFilter(enriched, snap: snap)
                    print("🧩 [WL] mergeAndFilter in=\(enriched.count) out=\(filtered.count)")
                    ...
                    print("❌ [WL vehicles] error: \(error)")
                    applyIfCurrent(epoch: epoch) { self.buses = [] }
                }

            // ----------------------------
            // 🇰🇷 MOTIE / 🇰🇷 Daejeon / 🇳🇿 Auckland
            // ----------------------------
            default:
...
// enum BusProvider { ... }  ← 교체
enum BusProvider { case motie, daejeon, auckland, wellington, christchurch }

// private let provider ...   ← 교체
private let provider: BusProvider = .christchurch
...
struct ClusteredMapView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        // ✅ 무조건 대전 시청에서 시작
        let startCenter = CLLocationCoordinate2D(latitude: -43.5321, longitude: 172.6362)
    ...
    final class Coord: NSObject, MKMapViewDelegate {
        ...
        // ClusteredMapView.Coord 내부 메서드 교체
        private let annotationWorkQueue = DispatchQueue(label: "map.annotation.queue", qos: .userInteractive)
        ...
        func applyAnnotationDiff(
            ...
            annotationWorkQueue.async { [weak self, weak mapView] in
                guard let self, let mapView else { return }

                // 0) 현재 맵에 있는지 스냅샷 (메인 필요)
                var present = Set<ObjectIdentifier>()
                DispatchQueue.main.sync {
                    present = Set(mapView.annotations.map { ObjectIdentifier($0) })
                }
                ...
                // 실제 맵에 있는 것만 제거 후보로, 팔로우/선택/업데이트 중은 제외
                let selectedIds: Set<String> = {
                    var s = Set<String>()
                    DispatchQueue.main.sync {
                        s = Set(mapView.selectedAnnotations.compactMap { ($0 as? BusAnnotation)?.id })
                    }
                    return s
                }()
                ...
                let safeBusesToRemove: [MKAnnotation] = busesToRemove.compactMap { a in
                    ...
                    if followedId == b.id { return nil }
                    if selectedIds.contains(b.id) { return nil }
                    if updatingBusIds.contains(b.id) { return nil }
                    return b
                }

                // 1) 제거 전에 좀비 표시 + 맵 플래그 false
                DispatchQueue.main.sync {
                    (safeStopsToRemove + safeBusesToRemove).forEach { a in
                        if let b = a as? BusAnnotation {
                            b.markZombie()
                            b.setOnMap(false)
                        }
                    }
                ...
                DispatchQueue.main.sync {
                    ...
                    // 추가된 버스는 onMap = true 로 전환
                    busesToAdd.forEach { a in (a as? BusAnnotation)?.setOnMap(true) }
                    CATransaction.commit()
                }

                // 4) 모델 업데이트 (좌표 KVO는 여기서만, 좀비/맵밖이면 skip)
                DispatchQueue.main.sync {
                    CATransaction.begin()
                    CATransaction.setDisableActions(true)
                    for (anno, live) in busUpdates {
                        // 제거되었거나 내려간 상태는 자동 skip
                ...
                // 5) 뷰 후처리(버블/틴트/겹침해소)
                DispatchQueue.main.sync { [weak self, weak mapView] in
                    guard let self, let mapView else { return }
...
struct StopETAInfoPanel: View {
    ...
    // 1) 순수 로직: ETA 압축
    private func compactETAs(_ src: [ArrivalInfo],
                             maxPerRoute: Int = 2,
                             capMinutes: Int = 40) -> [ArrivalInfo] {
        // 60분(또는 capMinutes) 초과 제거
        let filtered = src.filter { $0.etaMinutes <= capMinutes }

        // routeNo별 그룹
        let grouped = Dictionary(grouping: filtered, by: { $0.routeNo })

        // 각 노선에서 ETA 빠른 것 N개만 뽑기 (체이닝 대신 루프로 단순화)
        var result: [ArrivalInfo] = []
        let routes = grouped.keys.sorted()
        for r in routes {
            guard var arr = grouped[r] else { continue }
            arr.sort { $0.etaMinutes < $1.etaMinutes }
            result.append(contentsOf: arr.prefix(maxPerRoute))
        }

        // 전체도 ETA 오름차순으로
        result.sort { $0.etaMinutes < $1.etaMinutes }
        return result
    }

    // ✅ 에러 없이 컴파일 잘 되는 리스트 렌더링
    // StopETAInfoPanel 내부
    // StopETAInfoPanel.swift (뷰 내부)
    // 2) View 빌더: 명시적으로 View를 반환
    private func etaList() -> some View {
        let compact = compactETAs(vm.focusStopETAs, maxPerRoute: 2, capMinutes: 40)

        // 뷰는 반드시 "리턴" (some View)
        return VStack(alignment: .leading, spacing: 6) {
            ForEach(Array(compact.enumerated()), id: \.offset) { _, a in
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    Text(a.routeNo)
                        .bold()
                        .frame(minWidth: 44, alignment: .leading)

                    Text("\(max(0, a.etaMinutes))min")
                        .font(.callout)
                        .monospacedDigit()
...
struct JSONAPIList<T: Decodable>: Decodable {
    ...
    init(from decoder: Swift.Decoder) throws {
        ...
        if let c = try? decoder.container(keyedBy: CodingKeys.self),
           c.contains(.data),
           let arr = try? c.decode([JSONAPIResource<T>].self, forKey: .data) {
            self.data = arr
            return
        }
        ...
        // 2) 루트 배열 [T]
        if let sv = try? decoder.singleValueContainer(),
           let arr = try? sv.decode([T].self) {
            self.data = arr.enumerated().map { .init(id: String($0.offset), attributes: $0.element) }
            print("ℹ️ JSONAPIList: decoded root array [T], count=\(data.count)")
            return
        }
        ...
        if let c = try? decoder.container(keyedBy: CodingKeys.self),
           c.contains(.stops),
           let arr = try? c.decode([T].self, forKey: .stops) {
            self.data = arr.enumerated().map { .init(id: String($0.offset), attributes: $0.element) }
            print("ℹ️ JSONAPIList: decoded {stops:[T]}, count=\(data.count)")
            return
        }

        // 4) GeoJSON FeatureCollection { "features": [ { "properties": T, "id": ... } ] }
        // 4) GeoJSON FeatureCollection { "features": [ { "properties": T, "id": ... } ] }
        // JSONAPIList.init(from:) 안에서는 이렇게 사용
        if let c = try? decoder.container(keyedBy: CodingKeys.self),
           c.contains(.features),
           let feats = try? c.decode([GeoFeature<T>].self, forKey: .features) {
            self.data = feats.enumerated().map { f in
                .init(id: f.element.id ?? String(f.offset), attributes: f.element.properties)
            }
            print("ℹ️ JSONAPIList: decoded GeoJSON features, count=\(data.count)")
            return
        }


        // 5) 모든 경로 실패 → 원본을 남기기 위해 throw
        throw DecodingError.keyNotFound(CodingKeys.data,
            .init(codingPath: [], debugDescription: "No 'data'/'stops'/'features' and not a root array"))
    }
}

private struct GeoFeature<U: Decodable>: Decodable {
    let id: String?
    let properties: U
}
